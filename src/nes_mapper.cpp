...
mapper_nrom::mapper_nrom(vector<uint8_t> _prgrom, vector<uint8_t> _chrrom, vector<uint8_t> _trainer):
    prgrom(_prgrom),chrrom(_chrrom),trainer(_trainer){}
...
nes_mapper* nes_rom_loader::read_rom(){
    ...
    switch(mapper_id){
        case 0:
            mapper = new mapper_nrom(prgrom,chrrom,trainer);
